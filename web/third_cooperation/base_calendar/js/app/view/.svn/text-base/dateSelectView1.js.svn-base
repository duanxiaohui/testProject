...
*/
define(['zepto','amplify', 'monthView', 'weekView', '/js/lib/DatePicker_1/DateTimePicker.js'],function($, amplify, _mv, _wv) {
	var _data = {
		viewType: 'month'
	};
	return {
		...
		bindEvent: function () {
			...
			$("<div id='dtBox'></div>").appendTo('body').DateTimePicker({
				settingValueOfElement: function (sInputValue, dDateTime, oInputElement) {
					_data.viewType == 'week' ? amplify.publish("ev:changeDateWeek",dDateTime) : amplify.publish("ev:changeDate",dDateTime);
				},
