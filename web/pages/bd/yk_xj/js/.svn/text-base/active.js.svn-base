...
if(!app.getUa.weixin && app.getUa.android){
	...
	function loadResource(){
		...
		for (var i = 0; i < data.length; i++) {
			...
			for (var _i = 0; _i < num; _i++) {
				num == 1 ?
					loadImage('/pages/bd/yk_xj/images/'+active+'/' + (i + 1) + '.jpg') : 
					loadImage('/pages/bd/yk_xj/images/'+active+'/' + (i + 1) + '-' + (_i + 1) + '.jpg');
			};
		};

		loadImage('/pages/bd/yk_xj/images/'+active+'/'+active+'_color.jpg');
		loadImage('/pages/bd/yk_xj/images/'+active+'/'+active+'_black.jpg');
		// loadImage('/pages/bd/yk_xj/images/'+active+'/bir.png');
		// loadImage('/pages/bd/yk_xj/images/'+active+'/sign.png');
		var mp3Name = active;
		if(active == 'ln'){
			mp3Name = 'ln1'
		}
		if(!app.getUa.weixin && app.getUa.android){
			loadMp3('/pages/bd/yk_xj/mp3/20s/' + mp3Name + '.mp3?t=20150403_1');
		}
		else{
			loadMp3('/pages/bd/yk_xj/mp3/' + mp3Name + '.mp3?t=20150403_1');
		}
	}
	...
	function showStart () {
		...
		$('.start_btn, .txt_btn').on('tap', function () {
			$('audio')[0].play();
			// $('.music').css({
	...
	function setList () {
		var html = template(temp, active_data, {
			imgs: function (o, p, d, index) {
				...
				for (var i = 0; i < num; i++) {
					num == 1 ?
						imgHTML.push('<img class="img" src="/pages/bd/yk_xj/images/'+active+'/' + (index + 1) + '.jpg" alt="">') : 
						imgHTML.push('<img class="img" src="/pages/bd/yk_xj/images/'+active+'/' + (index + 1) + '-' + (i + 1) + '.jpg" alt="">')
				};
				return imgHTML.join('');
			}
		});
	...
	function startAnimation() {
		$('\
		<img class="photo_black hide" width="100%" src="/pages/bd/yk_xj/images/'+active+'/'+active+'_black.jpg" alt="">\
	    <img class="photo_color" width="100%" src="/pages/bd/yk_xj/images/'+active+'/'+active+'_color.jpg" alt="">\
	    ').appendTo('.photo');
	    setTimeout(function () {
	...
	function setEnd () {
		...
		var div = $('\
			<div class="scrollBg">\
				<div class="scroll_inner">\
					<div><img class="logo animated" src="/pages/bd/yk_xj/images/logos.png" width="225" height="85" alt="" /></div>\
					<div style="margin-top:10px;"><img class="animated none slogen1" src="/pages/bd/yk_xj/images/slogen1.png" width="191" height="41" alt="" /></div>\
		...
		$('.downLoad').on('tap', function () {
			$.ajax({
				url: '/operation/share.do?shareId=111&channel=weixin'
			})
			app.open('coco://365rili.com',app.getUa.ios,null,false);
		})
	...
	function share () {
		if(!app.getUa.weixin && getURLParameter('isShowbar') != 1){
			$.ajax({
				url: '/operation/share.do?shareId=110&channel=hot'
			});
			app.call({
	        	action: 'share',
	        	params: [
	        		{
	        			...
	        			value: JSON.stringify({
	        				title: '那些年我们追过的《仙剑》- 逍遥&月如重聚余杭镇',
	        				content: '《仙剑》二十年，逍遥月如余杭镇大团圆',
	        				link: 'http://www.365rili.com/pages/bd/yk_xj/weixin/active.html?active=qd',
	        				image: 'http://www.365rili.com/pages/bd/yk_xj/images/wx.jpg',
	        				isEvent: 'true'
	        			})
	        		}
	        	],
		...
		$.ajax({
			url: '/operation/share.do?shareId=110&channel=plaza'
		});
		$.ajax({
