					...
					success: function (data) {
						for (var i = 0; i < data.length; i++) {
							if (data[i].is_primary == "true" && data[i].data_domain != 'google'){
								...
								postData.updateV2Origin = '17';
								$.ajax({
									url: '/schedule/updateV2.do',
									data: postData,
									complete: function () {
										window.location.href = '/pages/bd/MidAutumn-2015/w/preview.html?' + ['to=' + to, 'from=' + from, 'job=' + job, 'customer=' + customer, 'beginDate=' + beginDate, 'endDate=' + endDate].join('&');
									}
								});
								return;
							}
