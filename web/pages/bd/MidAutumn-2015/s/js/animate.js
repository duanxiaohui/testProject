	...
	var _data = {
		...
		<div class="setups pages">\
			<section class="page setup setup0">\
				...
				<img class="openLettle animated infinite" src="/pages/bd/MidAutumn-2015/images/openLettleLow.png" width="56.4%" alt="" />\
	...
	var ani = {
		...
		init: function () {
			...
			if(!to){
				...
				$('.openLettle').on('tap', function () {
					window.location.href = 'http://www.365rili.com/pages/bd/MidAutumn-2015/s/case.html?' + ['to=' + to, 'from1=' + from, 'job=' + job, 'customer=' + customer, 'beginDate=' + beginDate, 'endDate=' + endDate].join('&');
				});
			}
		},
		setSize: function () {
		...
		setup4: function () {
			...
			$('.setup4Btn').on('tap', function () {
				if(app.getUa.weixin){
					window.location.href = "/wx/login.do?redURL=" + encodeURIComponent('/pages/bd/MidAutumn-2015/s/form.html');
				}
				else{
					window.location.href = '/pages/bd/MidAutumn-2015/s/form.html';
				}
			});
		},
