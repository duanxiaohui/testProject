	...
	var q = {
		...
		form: function () {
			$('.input').on('tap', function () {
				setTimeout(q.showInput.bind(this), 100);
			});
					...
					success: function (data) {
						for (var i = 0; i < data.length; i++) {
							if (data[i].is_primary == "true" && data[i].data_domain != 'google'){
								...
								postData.updateV2Origin = '15';
								$.ajax({
									url: '/schedule/updateV2.do',
									data: postData,
									complete: function () {
										window.location.href = '/pages/bd/MidAutumn-2015/app/preview.html?' + ['to=' + to, 'from=' + from, 'job=' + job, 'customer=' + customer, 'beginDate=' + beginDate, 'endDate=' + endDate].join('&');
									}
								});
								return;
							}
