...
//              '-webkit-animation': anim +' '+ duration + 'ms ' + timfunc + ' '+ delay + 'ms both',
    ...
    // 延迟动画 liumingren
    function aniDelay($element,$pram,i) {
        if(!$element.length)return;
        var $animation  = $pram[i].animation,
            $duration    = $pram[i].duration || 500,
            $timfunc     = $pram[i].timfunc || 'ease',
            $delay       = $pram[i].delay&&i==0 ? $pram[i].delay : 0;


        if (i < $pram.length-1) {
            i++;
            //计算延迟时间
            $element.on('webkitAnimationEnd webkitTransitionEnd',function(e) {
                setTimeout(function() {
                    aniDelay($element,$pram,i);
                },$pram[i].delay);
            });
            // var delayNext = $pram[i].delay ? parseInt($pram[i].delay) + parseInt($duration) : 0;
        ...
        //设置动画css
        setAnim($element,$animation,$duration,$timfunc,$delay);

    };

    //jquery调用
    $.fn.anim = function ( options ) {
        var prams = [];
        if(Object.prototype.toString.call( options ) === '[object Array]') {
            aniDelay( this, options, 0 );
        }else {
            prams.push(options);
            aniDelay( this, prams, 0 );
        }
        return $(this);
    };
    ...
    function animShow() {
        ...
        $animateDom.length && $animateDom.css({
        ...
        $('.current [data-animation]').each(function(index, element){
            var $element    = $(element),$pram,i;
            if($element.attr('data-animation')) {

                $pram= eval('(' + $element.attr('data-animation') + ')');
                // $pram= JSON.parse($element.attr('data-animation'));
                i = 0;
