...
*/
define(['zepto','util','monthInfo','wx','wxProtocol'],function($,_util,_mi,wx,wxProtocol) {
	return {
		...
		status: false,
		init:function() {
			var initDate = _util.query('date',location.href).replace(/-/g, '/');
		...
		renderMonthOther: function (date) {
			...
			//给黄历区域动态加载高度
			var wh = $(window).height(), bh = $('body').height(), ah = $('.almanac').height();
			if(wh>bh){
				$('.almanac').css({'height': wh-bh+ah+9+'px'});
			}
		},
		...
		moveTo: function(next, anim) {
			...
			//更新月份列表
			if( next == 0 || next < 0) {
				var date = new Date(domList1.find("li").eq(7).attr("data-date").substring(0,8)+'01');
		...
		bindEvent: function () {
			...
			//临时控制是否滑动
			if(!this.isNotScroll){
				$(document).on('touchstart', ".calendar_panel" , function(e) {
					//e.preventDefault();
					//e.stopPropagation();
					self.startX = e.targetTouches[0].pageX;
					self.startY = e.targetTouches[0].pageY;
					$(document).on('touchmove', ".calendar_panel" , function(e) {
						if(Math.abs(e.changedTouches[0].pageX-self.startX)>40){
							e.preventDefault();
							e.stopPropagation();
						}
					});
				...
				$(document).on('touchend', ".calendar_panel" , function(e) {
					e.preventDefault();
					e.stopPropagation();
					//$(document).off('touchstart', ".calendar_panel ul");
					var subX = e.changedTouches[0].pageX - self.startX;
					var subY = e.changedTouches[0].pageY - self.startY;
					var der = (subX > 40 || subX < -40) ? subX > 0 ? -1 : 1 : 0;
					if(Math.abs(subY)<30){
						self.moveTo($(this).index()+der, true);
					}
					//$(document).on('touchstart', ".calendar_panel ul");

				});

				$(".calendar_top_rightArrow").on("tap",function(e){
					e.preventDefault();
					e.stopPropagation();
					self.moveTo(2, true);
				});
				$(".calendar_top_leftArrow").on("tap",function(e){
					e.preventDefault();
					e.stopPropagation();
					self.moveTo(0, true);
				});
