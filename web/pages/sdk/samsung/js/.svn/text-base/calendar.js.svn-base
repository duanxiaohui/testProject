...
(function () {
	window._data = {
		...
		'temp': '\
			<dl class="e_clear js-schedule active_ce0 {$hasLogo}" data-uuid="{$uuid}" data-cid="{$cid}" data-id="{$id}" data-title="{$title}">\
			...
			success: function (datas) {
				...
				var subTmpl = $('\
				    <div class="subscribe_btn" data-focus="'+focusStatus+'" data-id="'+id+'">\
				        <div class="subscribe_icon '+(focusStatus ? 'on' : '')+'"><a href="javascript:;"></a></div>\
				        <p>'+focusStr+'</p>\
				    </div>');
				subTmpl.appendTo('body');
				subTmpl.on('tap', function () {
				    	if(!_data.key){
	...
	function getSchedules () {
		...
		$.ajax({
			...
			success: function (data) {
		        ...
		        $('.js-schedule').on('keyup', function (e) {
		        	if(e.keyCode == "13"){
		        		var _this = $(this);
						_fn.callAction('schedule_details', {
							cid: _this.data('cid'),
