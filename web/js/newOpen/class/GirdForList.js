define([
	...
	function Gird (dom, i) {
		...
		me.template = '\
					<dd class="e_clear" primary="{$primary}">\
						<a class="e_clear" href="http://www.365rili.com/pages/newOpen/template/schedule_detail.html?id={$id}">\
							<div class="schedule_img">\
								<img src="{$baseurl}{pic}"/>\
							</div>\
							<div class="schedule_txt">\
								<h3>{$title}</h3>\
								<p class="txt_p">{$text}</p>\
								<p>时间：{$time}</p>\
								{$location}\
							</div>\
						</a>\
	...
	Gird.prototype.updateScheduleView = function () {
		...
		for (var i = 0, l = scheduleLen; i < l; i++) {
			// console.log(me.schedule[i])
			calendar = cp.getCalendarById(me.schedule[i].cid);
			if(calendar.access_type == 2 || calendar.access_type == 3){
				temp = me.template;
			}
			else{
				temp = me.noEditTemplate;
			}
			html.push(template(temp, me.schedule[i], {
				// baseurl: 
				url: function (o, p) {
					return o ? '<p>链接：<a href="'+o+'" target="_blank">'+o+'</a></p>' : '';
				},
		...
		me.withOutHandle(function () {
			var dateDom = me.$dom.children();
			me.$dom.html(html.join(''));
			me.$dom.prepend(dateDom);
		});
	...
	Gird.prototype.updateDateView = function () {
		...
		//公历
		me.withOutHandle(function () {
			$dom.find('.month_view_ymd').text((me.date.getFullYear() + '年' + (me.date.getMonth()+1) + '月' + me.date.getDate() + '日').replace(/(\D)(\d)(?=\D|$)/g, '$10$2'));
		});

		me.withOutHandle(function () {
			$dom.find('.month_view_week').text(' 星期' + me.lunar.cnDay);
		});
		//节日||节气||农历
