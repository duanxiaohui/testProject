define([
	...
	var _ex = {
			...
			topic: '\
					<li>\
						<div class="list_top_img">\
							...
							<a href="###" class="cancel_top">取消置顶</a>\
							<div class="change_pic">\
								<a href="###" class="change_pic_btn">更换图片</a>\
								<p>建议尺寸640*480</p>\
							</div>\
						</div>\
						<p class="pic_alt">{$title}</p>\
					</li>'
		},
		...
		buildData: function () {
			_data.html = {
				day: '\
					<dl i={i} class="{evenodd} month_view_schedule" style="display:none;">\
						<dt><span class="month_view_ymd"></span><span class="month_view_week"></span></dt>\
					</dl>'
			};
		...
		buildBaseUI: function () {
			...
			//插入日表头
			html.unshift('<div id="div_lview" class="month_view_right">');
		...
		buildLogic: function () {
			...
			//显示今天
			$('#lnk_today').css('display', 'inline-block');
		...
		registGrid: function () {
			var girdDom = _data.gird = $('#div_lview dl');
			var gird = null;
		...
		getTopic: function () {
			$.ajax({
				url: '/schedule/getRotatePicById.do',
				type: 'GET'
			})
			.done(function(data) {
				data = eval('(' + data + ')');
		...
		showTopic: function (data) {
			console.log(data);
			var html = template(_ex.template.topic, data.activity, {
				baseurl: data.pic_path
			});
