...
$.DateTimePicker = $.DateTimePicker || {
	...
	defaults:  //Plugin Defaults
	{
        ...
        },  // formatHumanDate(oDate, sMode, sFormat)
...
$.cf = {
				...
				if(options === "destroy")
				{
					if(sArrDataKeys.length > 0)
					{
						for(iKey in sArrDataKeys)
						{
							...
							{
								$(document).unbind("tap.DateTimePicker");
	...
	DateTimePicker.prototype = {
		...
		_createPicker: function()
		{
			...
			$(".dtpicker-overlay").on('tap', function(e)
			{
				oDTP._hidePicker("");
			});
		...
		_modifyPicker: function()
		{
			...
			for(iTempIndex = 0; iTempIndex < iNumberOfColumns; iTempIndex++)
			{
				...
				sDTPickerComp += "<input type='number' readonly class='dtpicker-compValue'></input>";
		...
		_addEventHandlersForPicker: function()
		{
			...
			$(document).on("tap.DateTimePicker", function(e)
			{	
				oDTP._hidePicker("");
			});
			...
			{
				if(! $('.dtpicker-compValue:focus').length && parseInt(e.keyCode ? e.keyCode : e.which) === 9)
				{
			...
			$(".dtpicker-cont *").on('tap', function(e)
			{
				e.stopPropagation();
			});
			...
			$(oDTP.element).find('.dtpicker-close').on('tap', function(e)
			{
			...
			$(oDTP.element).find('.dtpicker-buttonSet').on('tap', function(e)
			{
				if(oDTP.settings.buttonClicked)
			...
			$(oDTP.element).find('.dtpicker-buttonClear').on('tap', function(e)
			{
				if(oDTP.settings.buttonClicked)
			...
			$(oDTP.element).find(".day .increment, .day .increment *").on('tap', function(e)
			{
			...
			$(oDTP.element).find(".day .decrement, .day .decrement *").on('tap', function(e)
			{
				oDTP.oData.iCurrentDay--;
				oDTP._setCurrentDate();
				oDTP._setOutputOnIncrementOrDecrement();
			...
			$(oDTP.element).find(".month .increment, .month .increment *").on('tap', function(e)
			{
			...
			$(oDTP.element).find(".month .decrement, .month .decrement *").on('tap', function(e)
			{
				oDTP.oData.iCurrentMonth--;
				oDTP._setCurrentDate();
				oDTP._setOutputOnIncrementOrDecrement();
			...
			$(oDTP.element).find(".year .increment, .year .increment *").on('tap', function(e)
			{
			...
			$(oDTP.element).find(".year .decrement, .year .decrement *").on('tap', function(e)
			{
				oDTP.oData.iCurrentYear--;
				oDTP._setCurrentDate();
				oDTP._setOutputOnIncrementOrDecrement();
			...
			$(oDTP.element).find(".hour .increment, .hour .increment *").on('tap', function(e)
			{
			...
			$(oDTP.element).find(".hour .decrement, .hour .decrement *").on('tap', function(e)
			{
				oDTP.oData.iCurrentHour--;
				oDTP._setCurrentDate();
				oDTP._setOutputOnIncrementOrDecrement();
			...
			$(oDTP.element).find(".minutes .increment, .minutes .increment *").on('tap', function(e)
			{
			...
			$(oDTP.element).find(".minutes .decrement, .minutes .decrement *").on('tap', function(e)
			{
				oDTP.oData.iCurrentMinutes -= oDTP.settings.minuteInterval;
				oDTP._setCurrentDate();
				oDTP._setOutputOnIncrementOrDecrement();
			...
			$(oDTP.element).find(".seconds .increment, .seconds .increment *").on('tap', function(e)
			{
			...
			$(oDTP.element).find(".seconds .decrement, .seconds .decrement *").on('tap', function(e)
			{
				oDTP.oData.iCurrentSeconds -= oDTP.settings.secondsInterval;
				oDTP._setCurrentDate();
				oDTP._setOutputOnIncrementOrDecrement();
			...
			$(oDTP.element).find(".meridiem .dtpicker-compButton").on('tap', function(e)
			{
