...
Calendar.prototype = {
	...
	getCalendarMonthRowCount: function(){
		...
		var y = date.getFullYear(), m = date.getMonth(), w = date.getDay();
		//4 rows
		if(m == 1 && w == 1 && !((y%4 == 0 && y%100 != 0) || y%400 == 0)){
			return 4;			
		}
		//6 rows
		if((w==6 && monthDate[m] == 31) || (w==0 && monthDate[m] >= 30)){
			return 6;			
		}
		//5 rows
		return 5;
	...
	getWeekFirstDate: function(){
		var date = new Date(+this.date);
		var day = date.getDay(), ar = [-6, 0, -1, -2, -3, -4, -5];
		date.setDate(date.getDate() + ar[day]);
		return date;
	},
	getWeekSundayFirstDate:function(){
		var date = new Date(+this.date);
		var day = date.getDay(), ar = [0, -1, -2, -3, -4, -5, -6];
		date.setDate(date.getDate() + ar[day]);
		return date;
	}
};
(function(){
	...
	function Lunar(oDate){
		 ...
		 * 修正1957年3月的农历
		 */
		if(this.oDate.getFullYear() == 1957){
			if(this.lMonth == '二'){
				if(this.lDate == '初一'){
					this.lMonth = '正';
					this.lDate = '三十';
				...
				else if(this.lDate == '卅一'){
					this.lMonth = '三';
					this.lDate = '初一';
					this.dateIndex = 0;
					this.lNextMonth = '四';
					this.isBigMonth = true;
				}
				else{
					this.lDate = DB.dateCn[days - 1];
				}
			}
			else if(this.lMonth == '三'){
				this.lDate = DB.dateCn[days + 1];
			}
		}
	}
