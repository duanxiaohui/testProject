(function(){
	...
	$.widget('dd.calendarPluginBirthday', {
		...
		_create: function(){
			...
			self.editHtml = '<li class="default {className}" noteid="{id}">\
                	<div class="birthday_list">\
						...
						<div class="difftime_txt"><span class="{difftime_day_color}">{difftime_day_text}</span><span class="{difftime_day_style}">天后</span> {difftime_year}岁生日   {difftime_month}月{difftime_date}日({difftime_week})</div>\
					</div>\
			...
			$elem.find('a.add_birthday_btn').click(function(evt){
				...
				$(".transform_solar").solarPicker();
				$(".transform_lunar").birthdaylunarPicker().hide();
			//$(".transform_solar").solarPicker('setDate',new Date());
			});
			...
			$(".add_birthday_date select").live("change",function(){
				var lunar_checkbox = $(this).find(".add_birthday_date #lunar_checkbox");
                if (lunar_checkbox.hasClass("on")) {
					var solarSelectDate=$(".transform_solar").find("select.create_day").find("option:selected").attr("date").split("-");
					var dds= new Date();
			...
			$('a.edit_birthady_btn', $elem).live('click', function(evt){
				...
				$li.addClass("edit");
				$li.find("input.input_name").val(datamap.name);
				if(datamap.sex=="男"){
		...
		difftimeDay:function(birthday_type,year,month,day){
			var date = this.getNextBirthdays(birthday_type,month,day), nowDate=new Date();
			if(!date){
				return {};
			}
		...
		getNextBirthdays:function(birthday_type,m,d){
			var date;
            var today = new Date();
			today.setHours(0);
			today.setMinutes(0);
			...
			if (birthday_type == "L") {
				for (var y = today.getFullYear(); y < 2051; y++) {
					if (monthDays(y, m) >= d) {
					...
					if (leapMonth(y) == m && leapDays(y) >= d) {
						date = getSolarDate(y, m, d, 1);
						if (date.getFullYear() > today.getFullYear() || date.getMonth()>today.getMonth()||
						(date.getMonth() == today.getMonth() && date.getDate() >= today.getDate()) ) {
							break;
						}
		...
		difftimeYear:function(birthday_type,year,month,day){
			var today =new Date(),dyear,solardate=getSolarDate(year,month,day), date = this.getNextBirthdays(birthday_type,month,day);
			dyear=date.getFullYear()-year;
			if(year==0){
				dyear=0;
			}
			return dyear;
