angular.module('365_calendar').controller('ScheduleCreator', ['$scope', 'ScheduleService','CalendarService', function($scope, ScheduleService, CalendarService){
	$scope.show = false;
	$scope.isSimple = true;
	...
	$scope.addSchedule = function(schedule){
		...
		ScheduleService.addSchedule(schedule, cid, $scope.isSimple).then(function(data){
			$scope.show = false;
		});		
	}
