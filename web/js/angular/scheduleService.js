angular.module('365_calendar').service( 'ScheduleService', ['$http', '$q', 'CalendarService' , function($http, $q, CalendarService ) {
	//variables
	var self = {
		dateMap:{},
	...
	function mergeScheduleData(data){
		...
		$.each(data, function(_, c){
			...
			$.each(expand_schlist, function(_, o){
				...
				o.colorStyle = CalendarService.getCalendarColorById(o.cid);
				if (self.dateMap[d]) {
	...
	var service = {
		...
		getWeekRows: function(date){
			...
			return $.map(new Array(7), function(_, j){
				var cell;
				var strDate = formatDate(startDate);
				if(self.dateMap[strDate]){
					var m = startDate.getMonth();
		...
		getStaticMonthRows:function(date){
			return service.getMonthRows(date, 6);
		},
		getMutableMonthRows:function(date){
			var cld = new Calendar(date)
			return service.getMonthRows(date, cld.getCalendarMonthRowCount())
		},
			...
			$.ajax({
				...
				success:function(data){
					if (data.state == 'wrongpass') {
						...
						//TODO:判断self.cids和data.cids是否一致
						//网络卡的情况下可能会出现bug
