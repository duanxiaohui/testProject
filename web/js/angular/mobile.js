angular.module('365_calendar', ['ngTouch']).
	controller('CalendarShareController', function($scope, $http, ScheduleService, CalendarService){
		
		CalendarService.getCalendarListWithExtend().then(function(data){
			if(data.state == "wrongpass"){
				location.href = "/account/login.do";
			}
			$scope.calendars = data;
			$scope.selectedCalendar = data[0];
			$scope.theme = getTheme(data[0].extend.theme);
			ScheduleService.setScheduleData(data[0].id).then(function(){});
		});
		...
		$scope.selectCalendar = function(calendar){
			...
			$scope.theme = getTheme(calendar.extend.theme);
			$scope.selectedCalendar = calendar;
			ScheduleService.setScheduleData(calendar.id).then(function(){});
		}
		function getTheme(skinJson){
			var theme = {};
