			...
			if(id=="#quit"){
				$.confirm("确定要安全退出此账户",{
					buttons: [{
						text: "退出",
						click: function(){
							location="/account/logout.do?redURL=/account/login.do";
						}
					}]
				});
				return;
...
function initFirstDayInWeek(){
	if($.cookie("calendar_panel_first") == "sunday"){
		$("#first_day_sunday").attr("checked","checked");
	}
	$(".first_day").change(function(){
		$.cookie("calendar_panel_first", $(this).val(), {
			expires:365,
			path:"/"
		});
	})
...
function initNotificationSetting () {
	if($.cookie("notification_setting") == "false" || Notification.permission != "granted"){
		$(".notification_setting").eq(1).prop("checked", true);
	}
	$(".notification_setting").change(function(){
		$.cookie("notification_setting", $(this).val(), {
			expires:365,
			path:"/"
		});
		if($(this).val() == "true"){
...
function setEmail(){
	var emailinput=$("#safe_email").val();
	var emailtxt=$('.email_txt').text();
	$(".edit_email_btn").click(function(){
		$(this).hide();
		$("#safe_email").removeClass("none");
		$(".email_txt").addClass("none");
		$(".setpassword").removeClass("none");
		$("#lnk_save_email").css('display','block');
	});
	if(emailinput != ''){
		$(".edit_email_btn").html("修改绑定邮箱");
	}else{
		$(".edit_email_btn").html("绑定邮箱");
	}
}
$(function($){
	showLoginInfo();
