define([
	...
	function Gird (dom, i) {
		...
		me.dateString = null;
		me.calendars = null;
		me.template = '<dd primary="{primary}" class="task jsc-{cid}" style="border-color:{color}; background-color:{bgc}"><div class="spheric ui-corner-all-16" style="border-color:{color}; background-color:{sbgc}"></div><p class="task_text {completed}" title="{time}&nbsp;{text}">{time}&nbsp;{text}</p></dd>';
	...
	Gird.prototype.init = function () {
		...
		me.$dom
			//绑定全部日程事件
			.on('click.Gird_' + me.index, '.task_more', function (e) {
				e.stopPropagation();
				me.showAllTask.call(me);
			})

			//绑定查看日程事件
			.on('click.Gird_' + me.index, '.task', function (e) {
				e.stopPropagation();
				me.showschedule.call(this);
			})

			//绑定新建日程事件
			.on('click.Gird_' + me.index, function (e) {
				e.stopPropagation();
	 ...
	 * 没用的函数
	 */
	// Gird.prototype.showOthers = function () {
	// 	console.log(123)
	...
	Gird.prototype.updateScheduleNum = function () {
		var me = this;

		//周试图不需要隐藏超出日程
		if(cp.getCurrentPanel() === 'week'){
			me.scheduleNum = me.schedule.length;
			me.more = false;
		}
		else{
	...
	Gird.prototype.updateScheduleData = function () {
		...
		//added by Xiaoqi
//		amplify.publish('currentScheduleUpdated', schedules); //
	...
	Gird.prototype.updateScheduleView = function () {
		...
		//增加显示日程
			...
			for (var i = me.shownScheduleNum, l = me.scheduleNum; i < l; i++) {
				...
				html.push($.format(me.template, schedule));
			};
	...
	Gird.prototype.setMore = function () {
		var me = this;
