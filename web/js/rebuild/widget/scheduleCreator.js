define([
	...
	amplify.subscribe('openSchedule', function () {
		if(!$form)return;
		$form.hide('fade');
	...
	function getForm() {
		if (!$form) {
			var tmpl = '<div id="div_add_schedule" class="add_schedule_layer complete ui-shadow none"><!--complete simple--><a href="javascript:;" class="close_btn js_close"></a><h2></h2><div class="add_schedule_content e_clear"><form><input type="hidden" name="scheduleId" value=""/><div class="createInput"><dl class="e_clear"><dt>内容：</dt><dd><textarea name="schTitle" placeholder="这里填写你的日程内容"></textarea></dd></dl></div>'+
				...
				'<div class="complete_time_calender"><dl class="e_clear"><dt>日历：</dt><dd><select name="calendarId"></select></dd></dl></div><div class="complete_content"><div class="complete_set s-no">' +
				'<dl class="e_clear"><dt>添加图片：</dt><dd><div id="image_upload_container" class="add_photo_layer"></div></dd></dl>' +
				'<dl class="e_clear"><dt>备注：</dt><dd><textarea name="schDescription" maxlength="3000"></textarea></dd></dl>' +
			...
			$form.dlt_start_hour.change(function(){
				if(getDuration() < 0){
					$form.dlt_end_hour.val($(this).val());
				}
			});

			$form.dlt_start_minute.change(function(){
				if(getDuration() < 0){
					$form.dlt_end_minute.val($(this).val());
				}
			});
			...
			$form.repeatCount.input({
				onInput: function(val) {
					...
					if(val !== 0){
						$form.repeatCount.val(val);
					}else{
						$form.repeatCount.val('');
					}
					summary();
				}
			});

			$form.repeatCount.blur(function () {
				var val = $form.repeatCount.val();
				if(val === ''){
					$form.repeatCount.val(20);
					summary();
				}
			})
	...
	function saveSchedule() {
		...
		if (postData.schTitle.length > 1000) {
			$.alert("日程内容字数不能超过1000，超出" + (postData.schTitle.length - 1000) + '字', {
				buttons: {
					'确定': function() {
						$(this).dialog("close");
						$form.schTitle.focus();
		...
		if (postData['description'].length > 3000) {
			$.alert("日程备注字数不能超过3000，超出" + (postData['description'].length - 3000) + '字', {
				buttons: {
					'确定': function() {
						$(this).dialog("close");
		...
		$.ajax({
			...
			success: function(data) {
				...
				if (data.state == 'ok') {
					...
					//$("#image_upload_container").hide('fade');
					pubScheduleCreated(postData.scheduleId, data);
	...
	function showForm(strDate, sid, $tds, scheduleContentStr, arDate) {
			amplify.publish('createSchedule');
			...
			if ($("#image_upload_container").length == 0) {
				$("body").append('<div id="image_upload_container" class="add_photo_layer"></div>');
			}
			...
			if (sid) {
				...
				if (!$tds || $tds.size() > 1) {
					...
					var rowIndex = $tds.prop('rowInex'),
						cellIndex = $tds.prop('cellIndex');
...
$("#image_upload_container").imageCreator("show");
