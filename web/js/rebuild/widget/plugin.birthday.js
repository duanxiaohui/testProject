define([
	...
	"rebuild/widget/birthday.lunarPicker",
	"rebuild/widget/solarPicker"
	],
	function(c){
	...
	$.widget('dd.calendarPluginBirthday', {
		...
		_create: function(){
			...
			self.editHtml = '<li class="default {className}" noteid="{id}">\
                    ...
                    <div class="birthday_edit">\
                        ...
                        <div class="phone_remind">\
                            <span>手机提醒：</span>\
                            <label>\
                                <span class="checkbox"></span><span class="remind_txt" alarms="0">生日当天</span>\
                            </label>\
                            <label>\
			...
			*/
			...
			$('a.edit_birthady_btn', $elem).live('click', function(evt){
				...
				$li.addClass("edit");
				$li.find("input.input_name").val(c.htmlDecode(datamap.name));
				if(datamap.sex=="男"){
		...
		_renderNewBirth : function($li, $c, data){
            ...
            //html编码
			$c.name = self.html_encode(self.html_decode($c.name));
		...
		difftimeYear:function(birthday_type,year,month,day){
			var dyear , ldateY= birthday_type == 'S' ? new Date(year, month - 1, day) : getSolarDate(year, month, day, 0),nextY=this.getNextBirthdays(birthday_type,year,month,day);
