...
function checkLogin()
{
	...
	$.ajax({
        type: 'post',
        data: {bd_user: bd_user, bd_sig: bd_sig},
        url: '/baidu/checkLogin.do',
        success: function(result) {
			if(result.login){
				//login(result.cid, result.username);
				if( !hasClearBaiduNotification ){
					clearBaiduNotification();
				}
...
function bd_loginOrAuth(){
	var callback_url = window.location.protocol  + "//" + window.location.host;
	var url = "https://openapi.baidu.com/oauth/2.0/authorize?response_type=code" +
				"&client_id=X34MNvmqSF32ZZFlDDVQHH2x" + 
				"&redirect_uri=" + callback_url + "/baidu/callback.do" +
				"&display=dialog";
...
//百度通知提醒清零
var hasClearBaiduNotification = false;
function clearBaiduNotification()
{
	$.ajax({
        type: 'post',
        data: {},
        url: '/baidu/clearBaiduNotification.do',
        success: function(result) {
			if(result == 'ok'){
				hasClearBaiduNotification = true;
			}
        },
        dataType: 'json'
    });
