var fangjiaData = {
    "yuandan": "1月1日放假。",
    "chunjie": "2月18日~2月24日放假，2月15日、2月28日（周六）上班。",
    "qingmingjie": "4月6日放假。",
    "laodong": "5月1日~5月3日放假。",
    "duanwu": "6月20日~6月22日放假。",
    "zhongqiu": "9月27日放假。",
    "guoqing": "10月1日~7日放假，10月10日上班。"
};
var festivalData = {
    "yuandan": "2015_1_1",
    "chunjie": "2015_2_18",
    "qingmingjie": "2015_4_5",
    "laodong": "2015_5_1",
    "duanwu": "2015_6_20",
    "zhongqiu": "2015_9_27",
    "guoqing": "2015_10_1"
...
var calander = {
    ...
    makeHuangli: function(date) {
        ...
        templates.lunar_Info_detail(date, showYJ);

        var _lunar = lunar(date);
        console.log(_lunar)

        $('#singleNumTd').text(date.getDate());
        $('#singleDateStr').text(date.getFullYear() + '年' + (date.getMonth() + 1) + '月 星期' + '日一二三四五六'.split('')[date.getDay()] + ' ');
        $('#festivalStr').text(templates.lunar_Info(date).festival);

        _lunar.term ? ($("#jieqiStr").text("24节气 - " + _lunar.term), $("#jieqiDiv").show()) : $("#jieqiDiv").hide();

        $('#nongliStr1').text(_lunar.gzYear + '年【' + _lunar.animal + '年】');
        $('#nongliStr2').text(_lunar.gzMonth + '月' + _lunar.gzDate + '日');
...
var templates = {
    ...
    lunar_Info_detail: function(date, callback) {
        ...
        if (year >= 2008 && year <= 2020){
            var huangliYearMonth = [year, _date.month].join('');

            if (!HuangLi['y' + huangliYearMonth]) {
        ...
        function doHuangli () {
            var hl = HuangLi['y' + huangliYearMonth][['d', _date.month, _date.date].join('')]
            info.huangliY = hl.y;
            info.huangliJ = hl.j;
